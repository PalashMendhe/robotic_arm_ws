...
env = os.environ.copy()
# We must source setup.bash. Since subprocess.Popen doesn't do that, we'll use bash -c
...
p_launch = subprocess.Popen(["bash", "-c", launch_cmd], stdout=open("/tmp/launch.log", "w"), stderr=subprocess.STDOUT)
...
p_pnp = subprocess.Popen(["bash", "-c", pnp_cmd], stdout=open("/tmp/pnp.log", "w"), stderr=subprocess.STDOUT)
